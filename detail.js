...
function updateTotals() {
  ...
  const totalComponentCostField = document.getElementById('totalComponentCost');
  if (totalComponentCostField) totalComponentCostField.textContent = formatCurrency(totalCost);
  const detailTargetCost = document.getElementById('detailTargetCost');
  if (detailTargetCost) detailTargetCost.textContent = formatCurrency(currentProject.targetCost);
  updateSellMargins(totalCost);
}

function populateSellOptions() {
  const sellLabel1 = document.getElementById('sellLabel1');
  const sellLabel2 = document.getElementById('sellLabel2');
  const sellLabel3 = document.getElementById('sellLabel3');
  const sellOption1 = document.getElementById('sellOption1');
  const sellOption2 = document.getElementById('sellOption2');
  const sellOption3 = document.getElementById('sellOption3');

  if (sellLabel1) sellLabel1.value = currentProject.sellLabels?.[0] || 'Option A';
  if (sellLabel2) sellLabel2.value = currentProject.sellLabels?.[1] || 'Option B';
  if (sellLabel3) sellLabel3.value = currentProject.sellLabels?.[2] || 'Option C';
  if (sellOption1) sellOption1.value = currentProject.sellOptions?.[0] || 0;
  if (sellOption2) sellOption2.value = currentProject.sellOptions?.[1] || 0;
  if (sellOption3) sellOption3.value = currentProject.sellOptions?.[2] || 0;
  updateTotals();
}
...
function updateSellMargins(totalCost) {
  ...
  const bestIndex = optionData.reduce((bestIdx, option, idx) => (option.marginValue > optionData[bestIdx].marginValue ? idx : bestIdx), 0);
...
function wireDetailEvents() {
  ...
  if (convertToRecurringBtn) {
    convertToRecurringBtn.addEventListener('click', () => {
      convertToRecurring();
    });
  }

  const placeNextOrderBtn = document.getElementById('placeNextOrderBtn');
  if (placeNextOrderBtn) {
    placeNextOrderBtn.addEventListener('click', () => {
      placeNextOrder();
    });
  }
  ...
  if (detailStageBadge && detailStageSelect) {
    detailStageBadge.addEventListener('click', () => {
      ...
      if (!detailStageSelect.classList.contains('hidden')) {
        detailStageSelect.focus();
      }
    });
  }
  ...
  const addComponentRow = document.getElementById('addComponentRow');
  if (addComponentRow) {
  ...
  ['1', '2', '3'].forEach((num) => {
    const el = document.getElementById(`sellLabel${num}`);
    if (!el) return;
    el.addEventListener('input', (event) => {
      if (!currentProject) return;
      currentProject.sellLabels = currentProject.sellLabels || ['Option A', 'Option B', 'Option C'];
